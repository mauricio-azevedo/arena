...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'group/card flex flex-col gap-4 rounded-[2rem] bg-card py-4 text-sm text-card-foreground shadow-[0_14px_34px_rgb(0_0_0/0.08)] ring-1 ring-white/[0.05] has-data-[slot=card-footer]:pb-0 data-[size=sm]:gap-3 data-[size=sm]:rounded-[1.5rem] data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0',
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex items-center bg-muted/50 p-4 group-data-[size=sm]/card:p-3', className)}
      {...props}
    />
  );
}
